...
Rails.application.configure do
  ...
  # Raise on a failed delivery. Mail goes out through deliver_later, so the
  # Solid Queue job absorbs the error and retries instead of failing a request.
  ...
  # alastor supplies from the wit-calendar-env agenix secret.
  config.action_mailer.delivery_method = :smtp
  config.action_mailer.smtp_settings = {
    address:              ENV.fetch("SMTP_ADDRESS", "smtp.resend.com"),
    port:                 ENV.fetch("SMTP_PORT", 587).to_i,
    user_name:            ENV.fetch("SMTP_USER_NAME", "resend"),
