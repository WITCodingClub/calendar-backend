...
RSpec.describe UserDegreeProgram do
  ...
  describe "enums" do
    it { is_expected.to define_enum_for(:status).backed_by_column_of_type(:string).with_values(active: "active", completed: "completed", dropped: "dropped", suspended: "suspended").with_default(:active) }
    it { is_expected.to define_enum_for(:program_type).backed_by_column_of_type(:string).with_values(major: "major", minor: "minor", certificate: "certificate", concentration: "concentration") }
  end
