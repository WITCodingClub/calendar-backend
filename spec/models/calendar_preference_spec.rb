...
RSpec.describe CalendarPreference, type: :model do
  ...
  describe 'validations' do
    ...
    it { should validate_length_of(:title_template).is_at_most(500) }
    it { should validate_length_of(:description_template).is_at_most(2000) }
  ...
  describe 'scopes' do
    ...
    describe '.global_scope' do
      it 'returns only global preferences' do
        expect(CalendarPreference.global_scope).to eq([global_pref])
      end
    end

    describe '.for_event_type' do
      it 'returns preferences for specific event type' do
        expect(CalendarPreference.for_event_type('lecture')).to eq([lecture_pref])
      end
    end
  end
  ...
  describe 'enums' do
    it 'defines scope enum' do
      expect(CalendarPreference.scopes).to eq({ 'global' => 0, 'event_type' => 1 })
    end
  end
end
